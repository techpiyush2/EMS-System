...
const constant = require("./constants"),
  ...
  secret = constant.cryptoConfig.secret,
...
//                         return res.json(Response(500, constant.messages.internalError, err));
//                     } else {
                ...
                for (var arrObjkey in data[key][j]) {
                  if (fields.indexOf(arrObjkey) == -1) {
                    if (data[key][j][arrObjkey]) {
                      patientObj[key][j][arrObjkey] = encrypt(
                        data[key][j][arrObjkey].toString()
                      );
                    }
                  } else {
                      ...
                      for (var arrObjkey in result[key][j]) {
                        if (fields.indexOf(arrObjkey) == -1) {
                          if (result[key][j][arrObjkey]) {
                            patientObj[key][j][arrObjkey] = decrypt(
                              result[key][j][arrObjkey].toString()
                            );
                          }
                        } else {
                          ...
                          if (result.length) {
                            if (fields.indexOf(arrObjkey1) == -1) {
                              if (result[key][objkey][arrObjkey1]) {
                                patientObj[objkey][arrObjkey1] = decrypt(
                                  result[key][objkey][arrObjkey1].toString()
                                );
                              }
                            } else {
                              patientObj[objkey][arrObjkey1] =
                                result[key][objkey][arrObjkey1];
                            }
                          } else {
                            if (fields.indexOf(arrObjkey1) == -1) {
                              if (result.objkey) {
                                patientObj.objkey = decrypt(
...
utility.ValidateRequiredFieldsInObj = function (obj, fields) {
  ...
  for (let i = 0; i < fields.length; i++) {
    if (
      typeof obj[fields[i]] === "boolean" &&
      !(obj[fields[i]] === false || obj[fields[i]] === true)
    ) {
      no_data_fields.push(fields[i]);
    } else if (
      typeof obj[fields[i]] === "number" &&
      !(obj[fields[i]] >= 0 || obj[fields[i]] < 0)
    ) {
  ...
  return no_data_fields.length
    ? "Required fields are missing!! Please pass " + no_data_fields.join(", ")
    : false;
};
...
utility.IsNotValidMongoIds = function (obj, object_id_fields) {
  ...
  for (let i = 0; i < object_id_fields.length; i++) {
    if (
      !Mongoose.Types.ObjectId.isValid(obj[String(object_id_fields[i]).trim()])
    )
      no_data_fields.push(object_id_fields[i]);
  }

  return no_data_fields.length
    ? "Invalid mongo id!! Please check " + no_data_fields.join(", ")
    : false;
};
