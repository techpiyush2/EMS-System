...
const Response = require('../../../../../lib/response'),
    ...
    { nationalityValidation } = require('./../../../../../lib/joiValidation'),
    Nationality = require('../models/nationality_model'),
    mongoose = require('mongoose')

exports.nationalityAdd = catchAsync(async (req, res, next) => {
    let validateObj = {
        title: req.body.title,
    }
    const userRes = await Nationality.findOne({
        title: req.body.title,
    })
    if (userRes) return res.json(Response(constants.statusCode.unauth, constants.messages.exist))

    await nationalityValidation.validateAsync(validateObj)
    ...
    let nationalityInfo = await Nationality.create(validateObj)
...
exports.nationalityList = catchAsync(async (req, res) => {
    ...
    const data = await Nationality.aggregate([
        { $match: condition },
        {
            $project: {
                title: '$title',
    ...
    const totalCount = await Nationality.countDocuments({
        ...condition,
        ...child_condition,
    })
...
exports.nationalityUpdate = catchAsync(async (req, res) => {
    const { _id } = req.body
    ...
    const Result = await Nationality.findByIdAndUpdate(_id, validateObj, {
        new: true,
    })
...
exports.nationalityDetail = catchAsync(async (req, res, next) => {
...
exports.changeStatus = toggleStatus(Nationality)

exports.delete = softDelete(Nationality)
